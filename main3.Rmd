---
title: "RIEM MAIN Period 3"
subtitle: "(Inspection Period: 10/01/2021 - 09/30/2022)"
date: "2024-09-04"
output:
  html_document:
    code_folding: hide
...
Year 0: 10/01/2020 - 09/30/2021
Year 1: 10/01/2021 - 09/30/2022 (Period of analysis)
...
#con <- odbcDriverConnect("Driver={Oracle in OraDB19Home1};Dbq=10.75.161.127:1526/AIDB;Uid=;Pwd=;")
```

```{r, eval = FALSE}
# Query Power Unit Counts for Each Carrier {-} 

# Year 0: 10/01/2020 - 09/30/2021 -> MCMIS201218
# Year 1: 10/01/2021 - 09/30/2022 (Period of analysis) -> MCMIS220325
# Year 2: 10/01/2022 - 09/30/2023 -> MCMIS230331

qry <-"
WITH CTE1 AS (SELECT a.DOT_NUMBER, b.CARRIER_EQUIPMENT_ID, EQUIPMENT_COUNT
FROM MCMIS230331.CENSUS a
    JOIN MCMIS230331.CARRIER_EQUIPMENT b ON a.DOT_NUMBER = b.DOT_NUMBER
    JOIN MCMIS230331.EQUIPMENT_TYPE c ON b.EQUIPMENT_TYPE_ID = c.EQUIPMENT_TYPE_ID
...
qry <-"
WITH CTE1 AS (SELECT a.DOT_NUMBER, b.CARRIER_EQUIPMENT_ID, EQUIPMENT_COUNT
FROM MCMIS220325.CENSUS a
    JOIN MCMIS220325.CARRIER_EQUIPMENT b ON a.DOT_NUMBER = b.DOT_NUMBER
    JOIN MCMIS220325.EQUIPMENT_TYPE c ON b.EQUIPMENT_TYPE_ID = c.EQUIPMENT_TYPE_ID
WHERE a.STATUS_CODE = 'A'
    AND b.STATUS_CODE = 'A'
    AND c.POWER_UNIT_FLAG = 'Y')
...
WHERE INSP_DATE BETWEEN TO_DATE('01/10/2021', 'DD/MM/YYYY') AND TO_DATE('30/09/2022', 'DD/MM/YYYY')
...
FROM MCMIS240830.INSPECTION
WHERE INSP_DATE BETWEEN TO_DATE('01/10/2020', 'DD/MM/YYYY') AND TO_DATE('30/09/2021', 'DD/MM/YYYY')
    AND DOT_NUMBER IS NOT NULL
GROUP BY DOT_NUMBER
...
WHERE REPORT_DATE BETWEEN TO_DATE('01/10/2022', 'DD/MM/YYYY') AND TO_DATE('30/09/2023', 'DD/MM/YYYY')
    AND DOT_NUMBER IS NOT NULL
GROUP BY DOT_NUMBER
...
FROM MCMIS240830.CRASH_MASTER
WHERE REPORT_DATE BETWEEN TO_DATE('01/10/2021', 'DD/MM/YYYY') AND TO_DATE('30/09/2022', 'DD/MM/YYYY')
    AND DOT_NUMBER IS NOT NULL
GROUP BY DOT_NUMBER
...
FROM MCMIS230331.CARRIER_DRIVER_COUNT
WHERE STATUS_CODE = 'A'
...
FROM MCMIS230331.ISSDP_FINAL NATURAL JOIN MCMIS230331.PRI_RUNINFO
WHERE PRZATION_RUNDATE BETWEEN TO_DATE('01/10/2017', 'DD/MM/YYYY') AND TO_DATE('30/09/2022', 'DD/MM/YYYY') ),
...
FROM MCMIS230331.INSPECTION NATURAL JOIN CTE1
WHERE INSP_DATE BETWEEN TO_DATE('01/10/2021', 'DD/MM/YYYY') AND TO_DATE('30/09/2022', 'DD/MM/YYYY')
    AND DOT_NUMBER IS NOT NULL
...
```{r}
summary <- data.frame(group_size, group_iss, slope, pval, n_carriers, mean_ir, mean_crc)

write_csv(summary, 'C:\\Users\\Philip.Situmorang\\Desktop\\RIEM\\1 - Main\\Period 3\\summary_slopes_period3.csv')
```
...
```{r}
summary1 <- data.frame(group, decile, ir_thresholds, crashes_pvtd, num_a_carriers,num_b_carriers, avg_crc_a, benchmark)
...
```
```{r}
summary2 <- data.frame(group, decile, ir_thresholds, crashes_pvtd, num_a_carriers,num_b_carriers, avg_crc_a, benchmark)
...
```{r}
summary3 <- data.frame(group, decile, ir_thresholds, crashes_pvtd, num_a_carriers,num_b_carriers, avg_crc_a, benchmark)
...
```{r}
summary4 <- data.frame(group, decile, ir_thresholds, crashes_pvtd, num_a_carriers,num_b_carriers, avg_crc_a, benchmark)
...
write_csv(sensitivity, 'C:\\Users\\Philip.Situmorang\\Desktop\\RIEM\\1 - Main\\Period 3\\sensitivity_period3.csv')
```
