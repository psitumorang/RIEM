...
This model estimates the effectiveness of roadside inspections for one fiscal year, or the period of analysis is detailed in the subtitle above. 
...
# Queries the number of crashes experienced by each carrier in period 2
...
# Queries the number of crashes experienced by each carrier in period 1
...
# driver to pu ratio
df1 <- df1 %>% mutate(driver_to_pu = df1$TOTAL_DRIVERS/df1$PU_COUNT)

# pre to post pu_count ratio
df1 <- df1 %>% mutate(pre_post_pu_ratio = df1$PU_COUNT_PRE/df1$PU_COUNT)
```
...
write_csv(sensitivity, 'C:\\Users\\Philip.Situmorang\\Desktop\\RIEM\\1 - Main\\Period 1\\sensitivity_period1.csv')
```

# Part 4 - Change in Inspection Rate vs Change in Crash Rate
